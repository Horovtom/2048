...
bool testStateGeneration1(Test &test) {
    ...
    test.assertTrue(containsState(gameStates, state, (1.0 / 3.0) * 0.75), "The result should contain this state!",
                    &chain);
    state = std::vector<int>({4, 8, 8, 0, 0, 4});
    test.assertTrue(containsState(gameStates, state, (1.0 / 3.0) * 0.25), "The result should contain this state!",
                    &chain);
    state = std::vector<int>({0, 8, 8, 2, 0, 4});
    test.assertTrue(containsState(gameStates, state, (1.0 / 3.0) * 0.75), "The result should contains this state!",
                    &chain);
    state = std::vector<int>({0, 8, 8, 4, 0, 4});
    test.assertTrue(containsState(gameStates, state, (1.0 / 3.0) * 0.25), "The result should contains this state!",
                    &chain);
    ...
    test.assertTrue(containsState(gameStates, state, 0.75), "The result should contain this state!", &chain);
    test.assertTrue(containsState(gameStates, {8, 4, 4, 2, 2, 4}, 0.25), "The result should contain this state!",
                    &chain);
