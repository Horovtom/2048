...
bool assertTrue(bool test, std::string assertName, bool* assertionChain) {
    std::cout << std::endl;
    if (!test) {
...
bool testMove3() {
    bool chain = true;
    assertTrue(1 == 1, "This shluld be fine");
    return chain;
}

bool runTest(TestFunction testFunction, std::string testName) {
